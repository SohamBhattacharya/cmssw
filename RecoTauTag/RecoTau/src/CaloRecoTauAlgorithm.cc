...
CaloRecoTauAlgorithm::CaloRecoTauAlgorithm(const edm::ParameterSet& iConfig, edm::ConsumesCollector &&iC) :
    ...
    chargedpi_mass_(0.13957018)
{
    ...
    EBRecHitsLabel = iConfig.getParameter<edm::InputTag>("EBRecHitsSource");
    EBRecHitsToken = iC.consumes<EBRecHitCollection>(edm::InputTag(EBRecHitsLabel));
    
    //EERecHitsLabel_                     = iConfig.getParameter<edm::InputTag>("EERecHitsSource");
    EERecHitsLabel = iConfig.getParameter<edm::InputTag>("EERecHitsSource");
    EERecHitsToken = iC.consumes<EERecHitCollection>(edm::InputTag(EERecHitsLabel));
    
    //ESRecHitsLabel_                     = iConfig.getParameter<edm::InputTag>("ESRecHitsSource");
    ESRecHitsLabel = iConfig.getParameter<edm::InputTag>("ESRecHitsSource");
    ESRecHitsToken = iC.consumes<ESRecHitCollection>(edm::InputTag(ESRecHitsLabel));
...
CaloTau CaloRecoTauAlgorithm::buildCaloTau(
    edm::Event& iEvent,
    const edm::EventSetup& iSetup,
    const CaloTauTagInfoRef& myCaloTauTagInfoRef,
    const Vertex& myPV
)
{
    //printf("Entering CaloRecoTauAlgorithm::buildCaloTau(...). \n");
    
    // catch a ref to the initial CaloJet 
    CaloJetRef myCaloJet = (*myCaloTauTagInfoRef).calojetRef(); 
    const std::vector<CaloTowerPtr> myCaloTowers = (*myCaloJet).getCaloConstituents();
    JetBaseRef jetRef = myCaloTauTagInfoRef->jetRef();
    
    // create the CaloTau with the corrected Lorentz-vector
    CaloTau myCaloTau(std::numeric_limits<int>::quiet_NaN(), jetRef->p4());
    ...
    // Get the leading track within dR < myMatchingConeSize w.r.t the jet axis
    ...
    if(myleadTk.isNonnull())
    {
        ...
        {
            // Propagate the leading track to the ECAL
            ...
            if(mypropagleadTrackECALSurfContactPoint.R() != 0.)
            {
                ...
                // Get the closest tower to the extrapolated point on the ECAL
                CaloTowerDetId mypropagleadTrack_closestCaloTowerId(
                    myCaloSubdetectorGeometry->getClosestCell(
                        GlobalPoint(
                            mypropagleadTrackECALSurfContactPoint.x(),
                ...
                // Get the neighbours of the closet tower
                std::vector<CaloTowerDetId> mypropagleadTrack_closestCaloTowerNeighbourIds = getCaloTowerneighbourDetIds(
                    myCaloSubdetectorGeometry,
                    *caloTowerTopology,
                    mypropagleadTrack_closestCaloTowerId
                );
                
                
                // Calculate quantites for the towers within a 3x3 window (the neighbourss) around the closest tower
                for(std::vector <CaloTowerPtr>::const_iterator iCaloTower  =myCaloTowers.begin(); iCaloTower != myCaloTowers.end(); iCaloTower++)
                {
                    ...
                    if (iCaloTowerId == mypropagleadTrack_closestCaloTowerId)
                    {
                    ...
                    {
                        for(std::vector<CaloTowerDetId>::const_iterator iCaloTowerDetId = mypropagleadTrack_closestCaloTowerNeighbourIds.begin();
                            iCaloTowerDetId!=mypropagleadTrack_closestCaloTowerNeighbourIds.end();
                            iCaloTowerDetId++
                        )
                        {
                    ...
                    // The hottest HCAL tower inside the window
                    if((**iCaloTower).hadEt() >= myleadTrackHCAL3x3hottesthitEt )
                    {
                        if((**iCaloTower).hadEt() != myleadTrackHCAL3x3hottesthitEt || 
                            ((**iCaloTower).hadEt() == myleadTrackHCAL3x3hottesthitEt &&
                                fabs((**iCaloTower).eta()-mypropagleadTrackECALSurfContactPoint.Eta()) < myleadTrackHCAL3x3hottesthitDEta
                            )
                        )
                        {
                            myleadTrackHCAL3x3hottesthitDEta = fabs((**iCaloTower).eta()-mypropagleadTrackECALSurfContactPoint.Eta());
                        }
                ...
                if(myleadTrackHCAL3x3hottesthitEt != 0.)
                {
                    myCaloTau.setleadTrackHCAL3x3hottesthitDEta(myleadTrackHCAL3x3hottesthitDEta);
                }
            }
        }
        
        // Store the tracks whose z impact parameters differs from that of the leading track by less than TrackLeadTrack_maxDZ_
        if(UseTrackLeadTrackDZconstraint_)
        {
            ...
            {
                if(fabs((**iTrack).dz(myPV.position())-myleadTkDZ) <= TrackLeadTrack_maxDZ_)
                {
                    myTksbis.push_back(*iTrack);
                }
            }
        ...
        // Store the signal tracks
        // All the tracks within dR < myTrackerSignalConeSize w.r.t. the leadign track
        TrackRefVector mySignalTks;
        
        if(UseTrackLeadTrackDZconstraint_)
        {
            mySignalTks = myCaloTauElementsOperators.tracksInCone(
                (*myleadTk).momentum(),
                TrackerSignalConeMetric_,
                myTrackerSignalConeSize,
                Track_minPt_,
        ...
        // Setting invariant mass and charge of the signal tracks system
        ...
        if((int)(mySignalTks.size()) != 0)
        {
            ...
            for(int i = 0; i < (int) mySignalTks.size(); i++)
            {
                ...
                math::XYZTLorentzVector mychargedpicand_fromTk_LorentzVect(
                    mySignalTks[i]->momentum().x(),
                    mySignalTks[i]->momentum().y(),
                    mySignalTks[i]->momentum().z(),
                    sqrt(std::pow((double)mySignalTks[i]->momentum().r(), 2) + std::pow(chargedpi_mass_, 2))
                );
        ...
        // Store the isolation tracks
        // All tracks with myTrackerSignalConeSize < dR < myTrackerIsolConeSize w.r.t. the leading track
        TrackRefVector myIsolTks;
        
        if (UseTrackLeadTrackDZconstraint_)
        {
            myIsolTks = myCaloTauElementsOperators.tracksInAnnulus(
                (*myleadTk).momentum(),
                TrackerSignalConeMetric_,
                myTrackerSignalConeSize,
                TrackerIsolConeMetric_,
                myTrackerIsolConeSize,
        ...
        for(int i = 0; i < (int) myIsolTks.size(); i++)
        {
            myIsolTks_Ptsum += myIsolTks[i]->pt();
        }
        ...
        //printf("Inside CaloRecoTauAlgorithm::buildCaloTau(...). Stage 1.0 \n");
        
        //iEvent.getByLabel(EBRecHitsLabel_,EBRecHits);
        iEvent.getByToken(EBRecHitsToken, EBRecHits);
        
        //printf("Inside CaloRecoTauAlgorithm::buildCaloTau(...). Stage 2.0 \n");
        
        //iEvent.getByLabel(EERecHitsLabel_,EERecHits);
        iEvent.getByToken(EERecHitsToken, EERecHits);
        
        //printf("Inside CaloRecoTauAlgorithm::buildCaloTau(...). Stage 3.0 \n");
        
        //iEvent.getByLabel(ESRecHitsLabel_,ESRecHits);
        iEvent.getByToken(ESRecHitsToken, ESRecHits);
        
        double maxDeltaR = 0.8;
        math::XYZPoint myCaloJetdir((*myCaloJet).px(), (*myCaloJet).py(), (*myCaloJet).pz());
        
        // Store the EB. EE, and ES hits which satisfy dR < maxDeltaR w.r.t. the calojet
        ...
        */
        ...
        std::vector < std::pair <math::XYZPoint,float> > myIsolPositionAndEnergyEcalRecHits = myCaloTauElementsOperators.EcalRecHitsInAnnulus(
            (*myleadTk).momentum(),
            ECALSignalConeMetric_,
            myECALSignalConeSize,
            ECALIsolConeMetric_,
            myECALIsolConeSize,
            ECALRecHit_minEt_,
            thePositionAndEnergyEcalRecHits
        );
        
        for(std::vector <std::pair <math::XYZPoint,float> >::const_iterator iEcalRecHit = myIsolPositionAndEnergyEcalRecHits.begin();
            iEcalRecHit != myIsolPositionAndEnergyEcalRecHits.end();
            iEcalRecHit++
        )
        {
    ...
    for(TrackRefVector::iterator iTrack = myTks.begin(); iTrack != myTks.end(); iTrack++)
    {
        ...
        math::XYZTLorentzVector iChargedPionCand_XYZTLorentzVect(
            (**iTrack).momentum().x(),
            (**iTrack).momentum().y(),
            (**iTrack).momentum().z(),
            sqrt(std::pow((double)(**iTrack).momentum().r(), 2) + std::pow(chargedpi_mass_, 2))
        );
    ...
    {
        if((*myCaloTowers[iTower]).hadEt() >= mymaxEtHCALtower_Et)
        {
            mymaxEtHCALtower_Et = (*myCaloTowers[iTower]).hadEt();
        }
    }
    
    myCaloTau.setmaximumHCALhitEt(mymaxEtHCALtower_Et);
    
    //printf("Exiting CaloRecoTauAlgorithm::buildCaloTau(...). \n");
