...
def getArrayFromTBranch(
    ...
    #    
    #    nObj = int(tree.Draw(varName, "", "goff", 1, eventNumber))
    #    
    #    if (not nObj) :
    #        
    #        continue
...
def plot1D(list_histDetails,
    ...
    if(legendPos == "UR") :
        ...
        legend = TLegend(0.185, 0.15, 0.15+legendWidth, 0.15+legendHeight)
    ...
    stack.Draw("%s goff" %(stackDrawOption))
...
def openTChain(listFileName, chain, treeName, nFileMax = -1, debug = False) :
    
    l_fileName = listFileName
    
    if ("list" not in str(type(listFileName))) :
    ...
    for iFile, fileName in enumerate(l_fileName) :
        ...
        if (1) :
            ...
            if (debug) :
                ...
                print("Adding to chain: %s" %(fileName_mod))
                #print("Adding to chain...")
            
            t1 = datetime.datetime.now()
            
            #chain.Add(fileName_mod)
            chain.AddFile(fileName_mod, -1, treeName)
            
            t2 = datetime.datetime.now()
            
            dt = t2-t1
            print("Time to add: %f" %(dt.total_seconds()))
