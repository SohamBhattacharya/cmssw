...
outFileName_sigEff = "%s/sigEff_%s" %(args.outDir, args.outFileName)
...
for iROC in range(0, len(args.varROC)) :
    ...
    key = "%s_%s" %(sigFiles, args.treeNames[iROC])
    
    if (key not in m_inputTree_sig) :
        ...
        for iTree, treeName in enumerate(l_treeName) :
            ...
            #    
            #    print("Adding sig file: %s" %(l_inFileName_sig[iFile]))
            #    tree_sig.AddFile(l_inFileName_sig[iFile], -1, treeName)
            
            Common.openTChain(
                listFileName = l_inFileName_sig,
                chain = tree_sig,
                treeName = treeName,
                nFileMax = -1,
                debug = True,
        ...
        m_inputTree_sig[key] = tree_sig
    
    else :
        
        tree_sig = m_inputTree_sig[key]
    
    
    bkgFiles = args.bkgFiles[iROC]
    tree_bkg = ROOT.TChain("chain_bkg")
    
    key = "%s_%s" %(bkgFiles, args.treeNames[iROC])
    
    if (key not in m_inputTree_bkg) :
        ...
        for iTree, treeName in enumerate(l_treeName) :
            ...
            ##for iFile in range(0, 10) :
            #    
            #    print("Adding bkg file: %s" %(l_inFileName_bkg[iFile]))
            #    tree_bkg.AddFile(l_inFileName_bkg[iFile], -1, treeName)
            
            Common.openTChain(
                listFileName = l_inFileName_bkg,
                chain = tree_bkg,
                treeName = treeName,
                nFileMax = -1,
                debug = True,
        ...
        m_inputTree_bkg[key] = tree_bkg
    
    else :
        
        tree_bkg = m_inputTree_bkg[key]
    ...
    l_eff_sig = numpy.zeros(len(l_discr))
    l_eff_bkg = numpy.zeros(len(l_discr))
    ...
    for iJob in range(0, len(l_job_sig)) :
        ...
        l_eff_sig[iJob] = l_job_sig[iJob].get()
        l_eff_bkg[iJob] = l_job_bkg[iJob].get()
    ...
    l_discr_unique = numpy.array(l_discr)[l_uniqueIndex]
    ...
    #    
    #    if (args.comparison[iROC] == "<") :
    #        
    #        l_discr_unique = numpy.append(l_discr_unique, min(l_discr_unique))
    #    
    #    elif (args.comparison[iROC] == ">") :
    #        
    #        l_discr_unique = numpy.append(l_discr_unique, max(l_discr_unique))
    #    
    #    #l_significance_unique = numpy.append(l_significance_unique, 0.0)
    #
    ## Add the x=1 point by hand if not already there
    #if (1 not in l_eff_sig_unique) :
    #    
    #    l_eff_sig_unique = numpy.append(l_eff_sig_unique, 1.0)
    #    #l_eff_bkg_unique = numpy.append(l_eff_bkg_unique, max(l_eff_bkg_unique))
    #    l_eff_bkg_unique = numpy.append(l_eff_bkg_unique, 1.0)
    #    
    #    if (args.comparison[iROC] == "<") :
    #        
    #        l_discr_unique = numpy.append(l_discr_unique, max(l_discr_unique))
    ...
    l_discr_unique = l_discr_unique[l_sortedIndex]
    l_eff_sig_unique = l_eff_sig_unique[l_sortedIndex]
    l_eff_bkg_unique = l_eff_bkg_unique[l_sortedIndex]
    
    fInter_sigEff = scipy.interpolate.InterpolatedUnivariateSpline(l_eff_sig_unique, l_discr_unique, bbox = [0, 1], k = 1, ext = "zeros")
    ...
    a_x = array.array("f", numpy.linspace(args.xMin, args.xMax, 1000))#[0: -1]
    a_y = array.array("f", [fInter_sigEff(ele) for ele in a_x])
    
    #print(list(zip(a_x[-10: ], a_y[-10: ])))
    
    gr_sigEff = ROOT.TGraph(len(a_x), a_x, a_y)
    h1_sigEff = Common.TGraphToTH1(graph = gr_sigEff, setError = False)
    
    h1_sigEff.SetName("sigEff_%d" %(iROC+1))
    
    #h1_sigEff.GetXaxis().SetRangeUser(0.0, 1.0)
    h1_sigEff.GetXaxis().SetRangeUser(0.0, max(a_x))
    
    histDetail_sigEff = Common.HistogramDetails()
    histDetail_sigEff.hist = h1_sigEff.Clone()
    histDetail_sigEff.hist.SetFillStyle(0)
    histDetail_sigEff.drawOption = "L"
    histDetail_sigEff.lineColor = args.lineColorList[iROC]
    histDetail_sigEff.lineStyle = args.lineStyleList[iROC]
    histDetail_sigEff.lineWidth = 3
    histDetail_sigEff.markerSize = 0
    #histDetail_sigEff.histLabel = "#splitline{#splitline{%s}{%s}}{AUC=%0.2g}" %(args.detailSig, args.detailBkg, areaROC)
    histDetail_sigEff.histLabel = "%s %s" %(args.labelList[iROC], args.comparison[iROC])
    histDetail_sigEff.histTitle = args.title
    
    
    l_histDetail_sigEff.append(histDetail_sigEff)
...
y1 = [h.hist.GetBinContent(h.hist.FindBin(args.xMin)) for h in l_histDetail_sigEff]
y2 = [h.hist.GetBinContent(h.hist.FindBin(0.99)) for h in l_histDetail_sigEff]

yMin_sigEff = min(y1 + y2)
yMax_sigEff = max(y1 + y2)

Common.plot1D(
    list_histDetails = l_histDetail_sigEff,
    stackDrawOption = "nostack",
    title = args.title,
    #titleSizeScale = 0.9,
    xTitle = "Signal efficiency", yTitle = "Discriminator threshold",
    xMin = args.xMin, xMax = args.xMax,
    yMin = yMin_sigEff, yMax = yMax_sigEff,
    ...
    outFileName = outFileName_sigEff.replace(".pdf", ""),
    outFileName_suffix = "",
)
