...
options.register("phoGenMatchDR",
    99999, # Default value
    VarParsing.VarParsing.multiplicity.singleton, # singleton or list
    VarParsing.VarParsing.varType.float, # string, int, or float
    "DeltaR to use for photon gen-matching (will store only the gen-matched ones)" # Description
)
...
process.treeMaker = cms.EDAnalyzer(
    ...
    phoGenMatchDR = cms.double(options.phoGenMatchDR),
    ...
    label_genJet = cms.InputTag("slimmedGenJets"),
    
    
    ############################## RECO ##############################
    
    label_primaryVertex = cms.InputTag("offlineSlimmedPrimaryVertices"),
    label_primaryVertex4D = cms.InputTag("offlineSlimmedPrimaryVertices4D"),
    
    label_pileup = cms.InputTag("slimmedAddPileupInfo"),
    label_rho = cms.InputTag("fixedGridRhoFastjetAll"),
    
    label_slimmedEle = cms.InputTag("slimmedElectrons"),
    #label_slimmedEle = cms.InputTag("slimmedElectrons", processName = cms.InputTag.skipCurrentProcess()),
    #label_slimmedEle = cms.InputTag("slimmedElectronsWithUserData"),
    #label_slimmedEle = cms.InputTag("updatedElectrons"),
    
    label_slimmedPho = cms.InputTag("slimmedPhotons"),
    #label_slimmedPho = cms.InputTag("updatedPhotons"),
    
    label_photonPhaseIImvaIdEB = cms.InputTag("photonPhaseIImvaIdEB"),
    
    #label_ElectronMVAEstimatorRun2Fall17IsoV2Values = cms.InputTag("electronMVAValueMapProducer", "ElectronMVAEstimatorRun2Fall17IsoV2Values"),
    #label_ElectronMVAEstimatorRun2Fall17IsoV2RawValues = cms.InputTag("electronMVAValueMapProducer", "ElectronMVAEstimatorRun2Fall17IsoV2RawValues"),
    #
    #label_ElectronMVAEstimatorRun2Fall17NoIsoV2Values = cms.InputTag("electronMVAValueMapProducer", "ElectronMVAEstimatorRun2Fall17NoIsoV2Values"),
    #label_ElectronMVAEstimatorRun2Fall17NoIsoV2RawValues = cms.InputTag("electronMVAValueMapProducer", "ElectronMVAEstimatorRun2Fall17NoIsoV2RawValues"),
    
    label_pfCandidate = cms.InputTag("packedPFCandidates"),
...
process.GenParticleFilter_ele.pdgIds = cms.int32(11)
...
process.GenParticleFilter_pho.label_generator = cms.untracked.InputTag("generator")
process.GenParticleFilter_pho.label_genParticle = cms.untracked.InputTag("prunedGenParticles")
process.GenParticleFilter_pho.atLeastN = cms.int32(1)
process.GenParticleFilter_pho.pdgIds = cms.vint32(22)
process.GenParticleFilter_pho.minEta = cms.double(0)
process.GenParticleFilter_pho.maxEta = cms.double(1.479)
process.GenParticleFilter_pho.isGunSample = cms.bool(bool(options.isGunSample))
#process.GenParticleFilter_ele.debug = cms.bool(True)

process.filterSeq_pho = cms.Sequence()

if (options.genPhoFilter) :
    
    process.filterSeq_pho = cms.Sequence(
        process.GenParticleFilter_pho
    )
...
#process.flashggPhotons = flashggPhotons.clone()
#process.flashggPhotons_seq = cms.Sequence(process.flashggPhotons)
...
process.p = cms.Path(
    ...
    process.filterSeq_pho *
    
    #process.egammaPostRecoSeq *
    #process.egammaPostRecoPatUpdatorSeq *
    
    #process.flashggPhotons_seq *
