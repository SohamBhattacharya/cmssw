...
class TreeMaker : public edm::one::EDAnalyzer<edm::one::SharedResources>
{
    ...
    edm::EDGetTokenT <std::vector <reco::PFCluster> > tok_TICLpfCluster;
    
    edm::EDGetTokenT <edm::ValueMap <edm::Ptr <reco::CaloCluster> > > tok_TICLpfClusterAssoc;
...
TreeMaker::TreeMaker(const edm::ParameterSet& iConfig)
{
    ...
    tok_TICLpfCluster = consumes <std::vector <reco::PFCluster> >(iConfig.getUntrackedParameter <edm::InputTag>("label_TICLpfCluster"));
    tok_TICLpfClusterAssoc = consumes <edm::ValueMap <edm::Ptr <reco::CaloCluster> > >(iConfig.getUntrackedParameter <edm::InputTag>("label_TICLpfClusterAssoc"));
...
void TreeMaker::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
    ...
    printf("Number of TICL tracksters: %d \n", (int) v_TICLtrackster->size());
    
    edm::Handle <std::vector <reco::HGCalMultiCluster> > v_TICLmultiCluster;
    iEvent.getByToken(tok_TICLmultiCluster, v_TICLmultiCluster);
    
    edm::Handle <std::vector <reco::PFCluster> > v_TICLpfCluster;
    iEvent.getByToken(tok_TICLpfCluster, v_TICLpfCluster);
    
    edm::Handle <edm::ValueMap <edm::Ptr <reco::CaloCluster> > > vmap_TICLpfClusterAssoc;
    iEvent.getByToken(tok_TICLpfClusterAssoc, vmap_TICLpfClusterAssoc);
    
    //for(int iTICLpfClus = 0; iTICLpfClus < (int) v_TICLpfCluster->size(); iTICLpfClus++)
    //{
    //    reco::PFCluster TICLpfClus = v_TICLpfCluster->at(iTICLpfClus);
    //    
    //    edm::Ptr <reco::PFCluster> TICLpfClus_ptr(v_TICLpfCluster, iTICLpfClus);
    //    
    //    edm::Ptr <reco::CaloCluster> caloClus_ptr = (*vmap_TICLpfClusterAssoc)[TICLpfClus_ptr];
    //    
    //    if(caloClus_ptr.isNonnull())
    //    {
    //        printf(
    //            "[%llu] "
    //            
    ...
    //            
    //            eventNumber,
    //            iTICLpfClus, (int) v_TICLpfCluster->size(),
    //            caloClus_ptr->energy()
    //        );
    //    }
    //}
    ...
    for(int iEle = 0; iEle < nEleFromTICL; iEle++)
    {
        ...
        if(storeSuperClusTICLclus)
        {
            ...
            {
                //edm::Ptr <reco::PFCluster> pfcluster = (*vmap_TICLpfClusterAssoc)[v_superClus_clus[iCluster]];
                ...
                //    "\n",
                //    
                //    iCluster, (int) v_superClus_clus.size(),
                //    cluster->energy(),
                //    pfcluster->energy()
                //);
            }
