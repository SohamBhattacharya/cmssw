...
void TreeMaker::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
    ...
    if(v_phoFromMultiClus.isValid())
    {
        ...
        for(int iPho = 0; iPho < (int) v_phoFromMultiClus_genPh_minDeltaR.size(); iPho++)
        {
            treeOutput->v_phoFromMultiClus_genPh_minDeltaR.push_back(v_phoFromMultiClus_genPh_minDeltaR.at(iPho));
            
            int index = v_phoFromMultiClus_matchedGenPh_idx.at(iPho);
            ...
            treeOutput->v_phoFromMultiClus_matchedGenPh_E.push_back(energy);
        }
    }
    ...
    std::map <reco::SuperClusterRef, int> m_pho_superClus;
    ...
    for(int iPho = 0; iPho < nPhoFromTICL; iPho++)
    {
        ...
        // PCA
        auto tup_phoFromTICL_superClus_PCAinfo = Common::getSuperClusPCAinfo(
            pho.superCluster(),
            m_recHit,
            &recHitTools,
            debug
        ...
        TMatrixD mat_phoFromTICL_superClus_rEtaPhiCov = std::get<0>(tup_phoFromTICL_superClus_PCAinfo);
        TVectorD v_phoFromTICL_superClus_rEtaPhiCov_eigVal = std::get<2>(tup_phoFromTICL_superClus_PCAinfo);
        
        treeOutput->v_phoFromTICL_superClus_sigma2rr.push_back(    mat_phoFromTICL_superClus_rEtaPhiCov(0, 0));
        treeOutput->v_phoFromTICL_superClus_sigma2etaEta.push_back(mat_phoFromTICL_superClus_rEtaPhiCov(1, 1));
        treeOutput->v_phoFromTICL_superClus_sigma2phiPhi.push_back(mat_phoFromTICL_superClus_rEtaPhiCov(2, 2));
        
        treeOutput->v_phoFromTICL_superClus_sigma2rEta.push_back(  mat_phoFromTICL_superClus_rEtaPhiCov(0, 1));
        treeOutput->v_phoFromTICL_superClus_sigma2rPhi.push_back(  mat_phoFromTICL_superClus_rEtaPhiCov(0, 2));
        treeOutput->v_phoFromTICL_superClus_sigma2etaPhi.push_back(mat_phoFromTICL_superClus_rEtaPhiCov(1, 2));
        
        treeOutput->v_phoFromTICL_superClus_sigma2diag1.push_back(v_phoFromTICL_superClus_rEtaPhiCov_eigVal(0));
        treeOutput->v_phoFromTICL_superClus_sigma2diag2.push_back(v_phoFromTICL_superClus_rEtaPhiCov_eigVal(1));
        treeOutput->v_phoFromTICL_superClus_sigma2diag3.push_back(v_phoFromTICL_superClus_rEtaPhiCov_eigVal(2));
        ...
        // Sorted hit energies
        std::vector <std::pair <int, double> > vp_phoFromTICL_superClus_sortedRecHit_index_energy = Common::getSortedHitIndex(
            pho.superCluster()->hitsAndFractions(),
            m_recHit
        );
        
        double phoFromTICL_superClus_recHit1_E = 0;
        double phoFromTICL_superClus_recHit2_E = 0;
        
        if(vp_phoFromTICL_superClus_sortedRecHit_index_energy.size() >= 1)
        {
            phoFromTICL_superClus_recHit1_E = vp_phoFromTICL_superClus_sortedRecHit_index_energy.at(0).second;
        }
        
        if(vp_phoFromTICL_superClus_sortedRecHit_index_energy.size() >= 2)
        {
            phoFromTICL_superClus_recHit2_E = vp_phoFromTICL_superClus_sortedRecHit_index_energy.at(1).second;
        }
        
        treeOutput->v_phoFromTICL_superClus_recHit1_E.push_back(phoFromTICL_superClus_recHit1_E);
        treeOutput->v_phoFromTICL_superClus_recHit2_E.push_back(phoFromTICL_superClus_recHit2_E);
        ...
        if(m_pho_superClus.find(pho.superCluster()) == m_pho_superClus.end())
        {
            m_pho_superClus[pho.superCluster()] = 0;
        }
        
        else
        {
            m_pho_superClus[pho.superCluster()]++;
        }
