...
namespace Common
{
    ...
    double getPileupDesity(
        edm::Handle <std::vector <PileupSummaryInfo> > pileUps_reco,
        reco::Vertex *pmVtx,
        double absWindow_z = 1.0
    )
    {
        double puDensity = 0;
        
        double pmVtx_z = pmVtx->z();
        ...
        for(int iPileUp = (int) pileUps_reco->size() - 1; iPileUp >= 0; iPileUp--)
        {
            ...
            for (int iZ = 0; iZ < (int) pileUpInfo.getPU_zpositions().size(); iZ++)
            {
                ...
                if(std::fabs(PU_z - pmVtx_z) < absWindow_z)
                {
                    puDensity++;
                }
            }
        ...
        // Get the # of vertices per mm
        puDensity /= (2.0*absWindow_z * 10.0);
