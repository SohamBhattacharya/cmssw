    ...
    */
    ...
    for(int iEle = 0; iEle < nEle; iEle++)
    {
        ...
        for(int iHit = 0; iHit < (int) v_superClus_HandF.size(); iHit++)
        {
            ...
            DetId hitEnfrac = v_superClus_HandF.at(iHit).second;
            ...
            double hitEn = v_PFRecHit->at(hitIdx).energy() * hitEnfrac;
            
            //const HGCRecHit *recHit = m_recHitPtr.at(hitId);
            //double hitEn = recHit->energy() * hitEnfrac;
            ...
            v_layerEnergy.at(hitLayer) += hitEn;
            
            v_layerCentroid.at(hitLayer) += hitEn * hit_xyz;
        }
        ...
        for(int iHit = 0; iHit < (int) v_superClus_HandF.size(); iHit++)
        {
            ...
            v_layerEnergyInR.at(hitLayer) += hitEn;
        }
        ...
        if(_debug)
        {
            printf("In HGCalElectronRvarProducer --> Ele %d/%d: Rvar %0.4f, totalE %0.4f \n", iEle+1, nEle, Rvar, totalE);
        }
